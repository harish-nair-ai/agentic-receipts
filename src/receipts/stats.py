...
def save_receipt(receipt: Receipt, config: Config) -> Path:
    ...
    stamp = receipt.timestamp.strftime("%Y%m%d_%H%M%S")
    file_path = sessions_dir / f"{stamp}_{receipt.receipt_id}.json"
...
def render_stats(receipts: list[Receipt], days: int = 7, file: TextIO = sys.stdout) -> None:
    ...
    bad_node = tree.add(
        Text(
            f"{bad_claims} unverified/refuted claims caught",
            style="yellow" if bad_claims > 0 else "green",
        )
    )
    ...
    rate_color = (
        "green" if verification_rate > 90 else "yellow" if verification_rate > 70 else "red"
    )
